...
class EfficientNetWrapper:
    ...
    def prepare_data(self):
        ...
        self.train_generator = DataGenerator(train_dir, self.config.BATCH_SIZE, self.classes, self.input_size, 
                                             augmentation=self.config.AU_LIST)
        self.val_generator = DataGenerator(val_dir, self.config.BATCH_SIZE, self.classes, self.input_size)
        self.test_generator = DataGenerator(test_dir, self.config.BATCH_SIZE, self.classes, self.input_size)
    ...
    def train(self):
        ...
        model.compile(optimizer=optimizer, loss=EfficientNetWrapper.loss_func(self.class_weights), metrics=['accuracy'])
