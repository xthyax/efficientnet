...
from efficientnet_module.modules.utils import count_class_KLA
...
def main(dataset_dir, input_size, gpu_count, checkpoint_path, last_epoch, efficientnet_architecture, batch_size, train_optimizer, continue_training, augmentation=None):
    ...
    for directory in [d for d in (os.path.join(dataset_path, d1) for d1 in os.listdir(dataset_path)) if os.path.isdir(d)]:
        if "train" in directory.lower():
            directory_sub = os.path.join(directory,"OriginImage")
            # print(directory_sub)
            count_class_KLA(directory_sub, dataset_path)
            # print("\\".join(directory_sub.split("\\")[:-1]))
            # directory_sub = os.path.join(directory,"TransformImage")
            # print(directory_sub)
            # count_class_KLA(directory)
        else:
            # print(directory)
            count_class_KLA(directory, dataset_path)
    _init_t =  input("Init train ?(Y/N)\nYour answer: ")
    if _init_t.lower() == "y":
        model.prepare_data_KLA(dataset_dir, batch_size= batch_size, augmentation=augmentation)
...
if __name__ == '__main__':
    ...
    last_epoch = 'last'
    main(DATASET_DIR, INPUT_SIZE, GPU_COUNT, CHECKPOINT_PATH, last_epoch, ARCHITECTURE, BATCH, Optimizer_, continue_training , augmentation=simple_augmentation)
